...
**2. 通过按需通信参数，执行与完整模型相同的计算**：**ZeRO和FSDP**属于这一类，优点是参数通信不依赖前面的计算，**在通信参数的同时，可以执行与所需参数无关的计算任务**；但要求单个设备的内存可以放下通信后的参数。![[Pasted-image-20260521223805.png]]
## FSDP流程![[Pasted-image-20260522113112.png]]
...
## FSDP单元unit
FSDP通过指定`wrap_policy`，模型实例分解为`Unit`单元![[Pasted-image-20260521224028.png]]
...
### 通算重叠![[Pasted-image-20260522113003.png]]
前向阶段：
